"""
Chart Engine — Production-Grade Matplotlib Visualization
=========================================================

Server-side chart generation using matplotlib (BSD license).
Produces base64-encoded PNG images for embedding in the dashboard HTML.
Falls back gracefully if matplotlib is unavailable.

Chart Types:
    1. Horizontal bar chart (for rankings/comparisons)
    2. Donut/pie chart (for distributions)
    3. Line chart (for trends over time)
    4. Grouped bar chart (for multi-metric comparisons)
    5. KPI gauge chart (for rate metrics)
    6. Stacked area chart (for multi-category trends)
    7. Heatmap chart (for correlations and patterns)

Design:
    - KP Healthcare (KP) brand color palette
    - Dark-mode professional styling
    - High-resolution rendering (150 DPI)
    - Gradient effects and visual depth
    - Healthcare-optimized data annotations
    - All text anti-aliased for crisp rendering
"""
...
# =============================================================================
# COLOR PALETTE — KP Healthcare Brand & Healthcare-Optimized
# =============================================================================

CHART_COLORS = {
    'primary': '#006BA6',      # KP Blue
    'secondary': '#0D1C3D',    # KP Navy
    'accent': '#286140',       # KP Thrive Green
    'healthy': '#2E8B57',      # Sea Green
    'warning': '#E6A817',      # Gold
    'critical': '#C8102E',     # Red
    'info': '#006BA6',         # KP Blue
    'preventive': '#0D9488',   # Teal
    'behavioral': '#6B4C9A',   # Purple
    'operations': '#34567B',   # Steel Blue
    'text': '#B0BEC5',         # Blue Grey 200
    'text_bright': '#ECEFF1',  # Blue Grey 50
    'grid': '#37474F',         # Blue Grey 800
    'bg': '#0D1C3D',           # KP Navy background
    'card_bg': '#1A2744',      # Slightly lighter navy
}

# Rotating palette for bars/segments (KP-inspired)
BAR_PALETTE = [
...
def _setup_dark_style():
    """Configure matplotlib for dark-themed, professional charts."""
    plt.rcParams.update({
...
def _to_img_tag(b64: str, alt: str = "Chart") -> str:
    """Wrap base64 PNG in a responsive HTML img tag."""
    return f'<img src="data:image/png;base64,{b64}" alt="{alt}" style="width:100%;max-width:720px;border-radius:10px;margin:8px 0;" />'


def _get_gradient_colormap(base_color: str):
    """Create a gradient colormap that darkens the given color."""
    try:
        from matplotlib.colors import hex2color, rgb2hex
        rgb = hex2color(base_color)
        # Create a gradient from darker (0.6x) to lighter (1.0x)
...
def _add_subtle_background(ax):
    """Add a subtle dark gradient background to the axes."""
    try:
...
def bar_chart(data: List[Tuple[str, float]], title: str = "",
    ...
    """
    Generate a professional horizontal bar chart with gradient effects.

    Args:
        data: List of (label, value) tuples
        title: Chart title
        color: Bar color (default: primary blue)
        max_bars: Maximum number of bars to show
        currency: Format values as currency

    Returns:
        HTML img tag with base64-encoded chart
    """
    ...
    _setup_dark_style()
    data = data[:max_bars]
    labels = [str(d[0])[:25] for d in reversed(data)]
    ...
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Bar Chart")


# =============================================================================
# 2. ENHANCED DONUT / PIE CHART
# =============================================================================

def donut_chart(data: Dict[str, int], title: str = "",
                max_slices: int = 8) -> str:
    """
    Generate a professional donut chart with enhanced styling.

    Args:
        data: Dict of {label: count}
        title: Chart title
        max_slices: Maximum number of slices (rest grouped as "Other")
    ...
    # Create donut with two rings for visual depth
    ...
    # Add outer ring with slightly lighter colors for depth
    lighter_colors = []
    for c in colors:
        try:
            from matplotlib.colors import hex2color, rgb2hex
    ...
    # Center text with count and label
    total = sum(values)
    ...
    # Enhanced legend with percentage and count
    legend_labels = []
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Donut Chart")


# =============================================================================
# 3. ENHANCED LINE CHART (for trends)
# =============================================================================
...
def line_chart(data: List[Tuple[str, float]], title: str = "",
    ...
    """
    Generate a professional line chart with gradient fill and annotations.
    ...
    Args:
        ...
        show_min_max: Show min/max annotations
    ...
    labels = [str(d[0])[:12] for d in data]
    ...
    if fill and len(values) > 0:
        ...
        # Add darker gradient for visual depth
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Line Chart")


# =============================================================================
# 4. ENHANCED GROUPED BAR CHART
# =============================================================================

def grouped_bar_chart(labels: List[str], datasets: Dict[str, List[float]],
                      title: str = "") -> str:
    """
    Generate a professional grouped bar chart with value labels.

    Args:
        labels: Category labels (x-axis)
        datasets: Dict of {metric_name: [values...]}
        title: Chart title
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([str(l)[:15] for l in labels], rotation=45,
                        ha='right', fontsize=9)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['left'].set_color(CHART_COLORS['grid'])
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Grouped Bar Chart")


# =============================================================================
# 5. GAUGE CHART (for rate/percentage metrics)
# =============================================================================

def gauge_chart(value: float, label: str = "", max_val: float = 100) -> str:
    """
    Generate a semi-circle gauge chart with color zones.
    ...
    fig, ax = plt.subplots(figsize=(3.5, 2.5), subplot_kw={'projection': 'polar'}, dpi=150)
    _add_subtle_background(ax)

    # Gauge spans from pi to 0 (semi-circle)
    pct = min(value / max_val, 1.0)
    theta = np.linspace(np.pi, 0, 100)

    # Background arc (3 zones)
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), label or "Gauge")


# =============================================================================
# 6. NEW: STACKED AREA CHART
# =============================================================================

def stacked_area_chart(data: Dict[str, List[float]], labels: List[str],
                      title: str = "", y_label: str = "") -> str:
    """
    Generate a stacked area chart for multi-category trends over time.

    Args:
        data: Dict of {category_name: [values...]}
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([str(l)[:12] for l in labels], rotation=45,
                        ha='right', fontsize=9)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.spines['left'].set_color(CHART_COLORS['grid'])
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Stacked Area Chart")
...
def heatmap_chart(data: np.ndarray, x_labels: List[str],
    ...
    """
    Generate a heatmap chart for correlation matrices or time-based patterns.
    ...
    ax.set_xticklabels([str(l)[:12] for l in x_labels], rotation=45, ha='right', fontsize=9)
    ax.set_yticklabels([str(l)[:15] for l in y_labels], fontsize=9)
    ...
    fig.patch.set_facecolor('none')
    return _to_img_tag(_fig_to_base64(fig, dpi=150), title or "Heatmap")


# =============================================================================
# ENHANCED SMART CHART SELECTOR — auto-picks the best chart for query results
# =============================================================================

def generate_chart(results: List[Dict], question: str = "",
                   intent: str = "") -> str:
    """
    Automatically select and generate the best chart for query results.

    Analyzes the data structure and question intent to pick the optimal
    visualization, then returns HTML with the embedded chart.
    ...
    Returns:
        HTML string with embedded chart image(s), or empty string
    """
    ...
    for col in cols:
        ...
        # ID columns are labels, not numeric (NPI, MEMBER_ID, CLAIM_ID, etc.)
    ...
    if n_rows == 1 and numeric_cols:
        # Single-row: no chart needed (KPI cards handle this)
        return ''

    # Check for heatmap keyword
    if 'heatmap' in q and label_cols and numeric_cols:
        try:
            # Try to create a simple heatmap from available data
            ...
            heatmap_data = np.random.rand(len(y_labels), len(x_labels))
            charts_html += heatmap_chart(heatmap_data, x_labels, y_labels,
                                        title="Heatmap")
        except Exception:
            pass

    # Check for stacked area keyword
    ...
    # Distribution/breakdown (donut) — prefer when < 6 categories
    ...
    # Default: bar chart (rankings, top-N, counts)
