"""Semantic Intelligence Layer for data-driven NL→SQL understanding with schema auto-discovery, TF-IDF indexing, and intent classification."""
...
class SparseVector:
    """Memory-efficient sparse vector for TF-IDF representations."""
    ...
    def cosine(self, other: 'SparseVector') -> float:
        if self._norm == 0 or other._norm == 0:
            return 0.0
        # Merge-intersection dot product — O(min(nnz_a, nnz_b))
        dot = 0.0
        i, j = 0, 0
...
class TFIDFIndex:
    """Pure-Python TF-IDF index with sublinear TF and IDF smoothing."""
    ...
    def search(self, query: str, top_k: int = 10, min_score: float = None) -> List[Tuple[str, float, Dict]]:
        """Search index by cosine similarity. Returns [(doc_id, score, metadata), ...]."""
...
class ColumnProfile:
    """Statistical profile of a single database column, learned from actual data."""
    __slots__ = (
...
class SchemaLearner:
    """Learns database schema by introspecting columns, types, and semantic profiles."""
    ...
    def learn(self, sample_size: int = 500) -> Dict[str, Any]:
        """Learn the complete schema by introspecting the database."""
        ...
        try:
            ...
            # ── Column Relationship Graph — structural column intelligence ──
            try:
    ...
    def _profile_table(self, conn: sqlite3.Connection, table: str, sample_size: int):
        """Profile all columns in a table by examining actual data."""
        try:
    ...
    def _infer_semantics(self, profile: ColumnProfile):
        """Infer semantic type from column name and sample values."""
    ...
    def _discover_joins(self):
        """Discover join relationships by finding shared column names across tables."""
    ...
    def get_date_columns(self, table: str) -> List[ColumnProfile]:
        """Get all date columns in a table."""
    ...
    def find_date_pair(self, table: str) -> Optional[Tuple[ColumnProfile, ColumnProfile]]:
        """Find a pair of dates that could represent a duration (start→end)."""
    ...
    def get_value_column(self, table: str, hints: List[str] = None) -> Optional[ColumnProfile]:
        """Find a likely 'value' column (currency, amount) in a table."""
        ...
        if hints:
            for n in numerics:
                ...
                if any(h in cn for h in hints):
                    return n
        # Default: first currency-tagged column, then first numeric
        for n in numerics:
...
class SemanticSchemaIndex:
    """TF-IDF searchable index over schema metadata for relevance ranking."""
    ...
    def _build_indexes(self):
        """Build semantic indexes from learned schema."""
        col_docs = []
    ...
    def match_columns(self, question: str, top_k: int = 15) -> List[Dict]:
        """Find columns that semantically match the question."""
    ...
    def match_tables(self, question: str, top_k: int = 5) -> List[Dict]:
        """Find tables that semantically match the question."""
    ...
    def match_values(self, question: str, top_k: int = 10) -> List[Dict]:
        """Find categorical values that match terms in the question."""
...
class IntentClassifier:
    """Classifies query intent using statistical feature vectors and nearest-centroid classification."""
    ...
    def _train(self):
        """Train centroids from exemplar questions."""
        # Build index from all exemplars
        docs = []
...
# --- Computed Column Inference ---

class ComputedColumnInferrer:
    """Infers computed columns from schema structure without hardcoded patterns."""
    ...
    def _infer(self):
        """Discover all computable columns from schema structure."""
        for table, profiles in self.learner.tables.items():
    ...
    def find_computed(self, question: str) -> List[Dict]:
        """Find computed columns relevant to a question."""
        q = question.lower()
        results = []
    ...
    def get_age_expr(self, table: str = None) -> Optional[Dict]:
        """Get age computation expression for a table."""
        for key, info in self.computed.items():
    ...
    def get_age_group_expr(self, table: str = None) -> Optional[Dict]:
        """Get age group expression for a table."""
        for key, info in self.computed.items():
...
class DataValidator:
    """
    Validates query feasibility by checking actual data.
    Instead of returning wrong results, tells the user when data doesn't exist.
    """
    ...
    def _profile(self):
        """Profile data ranges and known values."""
        conn = sqlite3.connect(self.db_path)
        try:
            for table, profiles in self.learner.tables.items():
                for p in profiles:
    ...
    def validate_sql(self, sql: str, tables: List[str]) -> Dict[str, Any]:
        """Validate that a SQL query will return meaningful results."""
        warnings = []
        suggestions = []
    ...
    def check_result_empty(self, sql: str) -> Tuple[bool, str]:
        """Execute SQL and check if results exist. Returns (has_data, message)."""
        conn = sqlite3.connect(self.db_path)
        try:
            ...
            # Check if single-row aggregation returned 0
...
# --- Adaptive Query Templates ---

class AdaptiveQueryBuilder:
    """Composes SQL queries from semantic understanding without hardcoded patterns."""
    ...
    def build_demographic_breakdown(self, fact_table: str, dim_table: str,
        ...
        """Build a multi-dimensional demographic breakdown query."""
    ...
    def build_rate_query(self, table: str, status_col: str, target_value: str,
        ...
        """Build a rate calculation query dynamically."""
    ...
    def build_age_group_query(self, table: str) -> Optional[str]:
        """Build an age group distribution query."""
...
class SemanticLayer:
    """Unified intelligence layer combining schema learning, indexing, classification, and query building."""

    def __init__(self, db_path: str, nlp_factory=None):
        self.db_path = db_path
        self.nlp_factory = nlp_factory  # Optional NLPEngineFactory for backend switching
        ...
        self.entity_extractor = None  # From NLPEngineFactory if available
        self._initialized = False

    def initialize(self) -> Dict[str, Any]:
        """Initialize by learning schema and training classifiers."""
        self.learner = SchemaLearner(self.db_path)
        schema_info = self.learner.learn()
        logger.info("Schema learned: %s", schema_info)
    ...
    def _enhance_classifier(self):
        """Enhance the intent classifier with factory-provided backends."""
        try:
            lib_clf = self.nlp_factory.get_classifier()
            if lib_clf.backend == 'scratch':
                return  # No upgrade available

            # Train the library classifier on the same exemplar data
            texts, labels = [], []
    ...
    def extract_entities(self, text: str) -> List[Dict]:
        """Extract entities using the factory-provided extractor."""
    ...
    def get_schema_summary(self) -> Dict[str, Any]:
        """Return a summary of the learned schema."""
        if not self._initialized:
            return {}
        summary = {}
        for table, profiles in self.learner.tables.items():
