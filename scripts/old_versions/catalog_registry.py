"""
Catalog Registry — Multi-Environment, Multi-Catalog Discovery System

Models the Databricks Unity Catalog hierarchy:
    Environment → Catalog → Schema → Table → Subtypes

Examples:
    prod → enriched_catalog → claims → inpatient_institutional
    prod → enriched_catalog → claims → outpatient_professional
    staging → staging_catalog → claims → inpatient_institutional
    dev → dev_catalog → claims → raw_claims

Also supports data source systems (DSW, Clarity, Apixio, etc.) as
top-level "sources" that map into catalogs.

Interactive discovery flow:
    User: "show me claims"
    Bot:  "I found claims tables in these catalogs: [enriched, staging].
           Which catalog? And which claim type: [inpatient_institutional,
           outpatient_professional, pharmacy, dental]?"

Thread-safe, JSON-configurable, auto-discovers from Databricks or local config.
"""
...
@dataclass
class TableInfo:
    """A single table/view in the catalog hierarchy."""
    name: str                           # e.g. "inpatient_institutional_claims"
    schema_name: str                    # e.g. "claims"
    catalog_name: str                   # e.g. "enriched_catalog"
    environment: str                    # e.g. "prod"
    source_system: str = ""             # e.g. "DSW", "Clarity", "Apixio"
    table_type: str = "TABLE"           # TABLE, VIEW, MATERIALIZED_VIEW
    description: str = ""
    row_count: Optional[int] = None
    columns: List[str] = field(default_factory=list)
    column_types: Dict[str, str] = field(default_factory=dict)
    tags: List[str] = field(default_factory=list)  # e.g. ["inpatient", "institutional", "claims"]
    parent_concept: str = ""            # e.g. "claims" — the broad concept this belongs to
    last_updated: str = ""
    is_active: bool = True

    @property
    def full_path(self) -> str:
        """Unity Catalog style: catalog.schema.table"""
        return f"{self.catalog_name}.{self.schema_name}.{self.name}"

    @property
    def display_path(self) -> str:
        """Human-readable: [env] catalog.schema.table"""
...
@dataclass
class CatalogInfo:
    """A catalog (database) within an environment."""
    name: str
    environment: str
    description: str = ""
...
class EnvironmentInfo:
    """An environment (prod, staging, dev, etc.)."""
    name: str
    description: str = ""
...
# =============================================================================
# CONCEPT HIERARCHY — maps broad concepts to specific table types
# =============================================================================

# Healthcare domain: when user says "claims", these are the possible subtypes
CONCEPT_HIERARCHY = {
    'claims': {
...
class CatalogRegistry:
    """
    Thread-safe registry of all environments, catalogs, schemas, and tables.

    Supports:
    - Auto-discovery from Databricks Unity Catalog
    - Local configuration from JSON
    - Interactive table discovery for chatbot
    - Concept-to-table mapping with hierarchy
    """
    ...
    def __init__(self):
        ...
        self._table_index: Dict[str, TableInfo] = {}  # full_path → TableInfo
        self._keyword_index: Dict[str, List[TableInfo]] = defaultdict(list)
        self._active_context: Dict[str, Dict] = {}  # user_id → {env, catalog, schema}
    ...
    def initialize(self, config_path: Optional[str] = None, auto_discover: bool = True):
        """Initialize from config file and optionally auto-discover from Databricks."""
        with self._lock:
            ...
            # 2. Auto-discover from local database
            self._discover_local_tables()

            # 3. Auto-discover from Databricks if available
            if auto_discover:
    ...
    def _discover_local_tables(self):
        """Discover tables from local SQLite database."""
        ...
        try:
            ...
            # Create local environment if not already configured
            env_name = 'local'
            ...
            for row in tables:
                ...
                # Determine parent concept
                parent = tbl_name.lower().rstrip('s')  # 'claims' → 'claim'
    ...
    def _discover_databricks(self):
        """Auto-discover tables from Databricks Unity Catalog."""
        try:
                                    ...
                                    try:
                                        ...
                                        if tables_result and tables_result.get('rows'):
                                            for t_row in tables_result['rows']:
                                                ...
                                                # Build tags from table name
                                                tags = [w for w in t_lower.replace('_', ' ').split() if len(w) > 2]
    ...
    def get_catalogs(self, environment: Optional[str] = None) -> List[Dict]:
        """List catalogs, optionally filtered by environment."""
        results = []
        for env in self.environments.values():
    ...
    def get_tables(self, concept: Optional[str] = None,
        ...
        If concept is given, returns all tables matching that concept across all catalogs.
        """
        results = []
    ...
    def search_tables(self, query: str) -> List[Dict]:
        """Search for tables by keyword across all catalogs and environments."""
    ...
    def _table_to_dict(self, t: TableInfo) -> Dict:
        return {
            ...
            'columns': t.columns[:20],  # first 20 columns
    ...
    def discover_for_query(self, question: str) -> Dict:
        """
        Analyze a user question and determine what catalog/table discovery is needed.

        Returns a discovery result with:
        - needs_clarification: bool
        - matched_concept: str or None
        - available_catalogs: list
        - available_subtypes: list
        - available_sources: list
        - suggested_table: TableInfo or None (if unambiguous)
        - clarification_prompt: str (if ambiguous)
        """
        q_lower = question.lower()

        # 1. Detect concept from question
        ...
        # 2. Find all tables matching this concept
        matching_tables = self.get_tables(concept=matched_concept)

        # 3. Check if user specified a subtype
        ...
        # 4. Check if user specified a catalog/environment/source
        matched_env = None
        ...
        # If only one table matches, no clarification needed
        ...
        # Only ask for clarification if the user explicitly referenced
        # catalog/env/source/subtype keywords — not for regular queries.
        # This prevents "total claims" from triggering catalog selection.
    ...
    def set_user_context(self, user_id: str, environment: Optional[str] = None,
        ...
        """Set the active catalog context for a user."""
...
class ContextualLearningEngine:
    """
    Learns from all user interactions to provide:
    - Per-user conversation history with context
    - "Other people also asked" suggestions
    - Query pattern learning (what users search for → what tables/columns are used)
    - Trending topics and popular queries

    Stored in SQLite for persistence across server restarts.
    """
    ...
    def log_query(self, user_id: str, username: str, question: str,
        ...
        try:
            ...
            # Auto-generate suggestion if query is popular enough
    ...
    def _normalize_question(self, question: str) -> str:
        ...
        # Remove specific values (numbers, dates, names)
    ...
    def _make_index_key(self, normalized: str) -> str:
        """Create a grouping key from a normalized question."""
    ...
    def _maybe_add_suggestion(self, conn, normalized: str, question: str):
        """Add as suggestion if popular enough (asked 3+ times)."""
        count = conn.execute(
    ...
    def get_suggestions(self, user_id: Optional[str] = None,
        ...
        try:
            ...
            # 1. Popular queries (from all users)
            ...
            # 2. User-specific recent queries
            if user_id:
                recent = conn.execute("""
            ...
            # 3. Trending (asked more in last 24h than usual)
    ...
    def get_similar_queries(self, question: str, limit: int = 5) -> List[Dict]:
        """Find similar queries that other users have asked."""
        normalized = self._normalize_question(question)
        index_key = self._make_index_key(normalized)
        ...
        conn = self._get_conn()
        try:
            # Find queries with overlapping index keys
    ...
    def set_context(self, user_id: str, session_id: str, key: str, value: Any):
        """Store a context value for a user session."""
        conn = self._get_conn()
        try:
            conn.execute("""
    ...
    def get_context(self, user_id: str, session_id: str) -> Dict:
        """Get all context for a user session."""
        conn = self._get_conn()
        try:
            rows = conn.execute(
