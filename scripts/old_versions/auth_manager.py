"""
Authentication & Dashboard Save/Load Manager

Production-ready user auth system with:
- bcrypt-compatible password hashing (falls back to hashlib if bcrypt unavailable)
- SQLite user store (portable, works in Databricks too)
- Team management
- Session tokens
- Saved dashboard/query persistence per user
"""
...
# Try bcrypt for production; fall back to hashlib PBKDF2
try:
    import bcrypt
    HAS_BCRYPT = True
except ImportError:
...
AUTH_DB_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), '..', 'data', 'auth.db'
)
...
def signup(username: str, password: str, display_name: str = "",
    ...
    # Email is optional — leave blank if not provided (no fake auto-generation)
...
def sso_login_or_create(email: str, display_name: str = "", provider: str = "",
    ...
    """
    Login or auto-create user from SSO provider data.
    If user with this email exists → login. Otherwise → create account.
    Returns (success, message, session_token_or_None).
    """
    if not email:
    ...
    try:
        ...
        if row:
            # Existing user — update SSO info and login
            conn.execute("""
            ...
            user_id = f"user_{secrets.token_hex(8)}"
            # SSO users get a random password (they authenticate via SSO)
...
def validate_session(session_token: str) -> Optional[Dict]:
    """
    Validate session token. Returns user info dict or None.
    """
...
def _create_session(conn, user_id: str) -> str:
    ...
    conn.execute(
        "INSERT INTO sessions (session_token, user_id, expires_at) VALUES (?, ?, datetime('now', '+7 days'))",
        (token, user_id)
    )
    return token
...
def save_dashboard(session_token: str, name: str, queries: List[Dict],
    ...
    """
    Save a dashboard (list of query results) for the authenticated user.
    Returns (success, message, dashboard_id).
    """
...
def get_team_dashboards(session_token: str) -> List[Dict]:
    """Get all saved dashboards for the user's team."""
    user = validate_session(session_token)
...
def set_export_password(session_token: str, password: str, label: str = "") -> Tuple[bool, str]:
    """Set/update the export password for the user (separate from login password)."""
    user = validate_session(session_token)
    if not user:
        return False, "Not authenticated"
    if not password or len(password) < 4:
        return False, "Export password must be at least 4 characters"

    conn = _get_conn()
    try:
        # Remove old export password
...
def verify_export_password(session_token: str, password: str) -> bool:
    """Verify the export password for the user."""
    user = validate_session(session_token)
    if not user:
        return False
    conn = _get_conn()
...
def has_export_password(session_token: str) -> bool:
    """Check if user has an export password set."""
    user = validate_session(session_token)
    if not user:
        return False
    conn = _get_conn()
...
def create_user_session(session_token: str, session_name: str = "") -> Tuple[bool, str, Optional[str]]:
    """Create a new query session for the user. Returns (success, message, user_session_id)."""
    user = validate_session(session_token)
    if not user:
        return False, "Not authenticated", None
    conn = _get_conn()
...
def update_user_session(session_token: str, user_session_id: str, queries: List[Dict]) -> Tuple[bool, str]:
    """Update a user session's queries."""
    user = validate_session(session_token)
    if not user:
        return False, "Not authenticated"
    conn = _get_conn()
...
def get_user_sessions(session_token: str) -> List[Dict]:
    """Get all query sessions for the authenticated user."""
    user = validate_session(session_token)
    if not user:
        return []
    conn = _get_conn()
