...
Centralizes all production-readiness concerns:
  1. Structured logging (replaces all print statements)
  2. Persistent database connection pool (replaces per-query CSV reload)
  3. Input validation & sanitization
  4. SQL safety (whitelist validation)
  5. Rate limiting
  6. Configuration management (auto-resolving paths)
  7. Health check data
  8. HIPAA-aware audit logging

All libraries used are Python stdlib — zero additional dependencies.
"""
...
def setup_logging(log_dir: str = None, level: str = "INFO",
    ...
    Creates two loggers:
      - 'mtp' — application logger (info, warnings, errors, debug)
      - 'mtp.audit' — HIPAA audit trail (query access, data access, errors)

    Args:
        log_dir: Directory for log files (default: ../logs relative to scripts/)
        level: Log level (DEBUG, INFO, WARNING, ERROR)
        enable_audit: Whether to create a separate audit log
    ...
    with _LOG_LOCK:
        ...
        # Restrict permissions on log directory (owner-only)
        try:
        ...
        # Console handler — structured format
        ...
        # File handler — rotating, max 10MB x 5 files
        app_log = os.path.join(log_dir, 'mtp_app.log')
        file_handler = logging.handlers.RotatingFileHandler(
            app_log, maxBytes=10_000_000, backupCount=5,
            encoding='utf-8'
        )
        ...
        if enable_audit:
            audit_logger = logging.getLogger('mtp.audit')
            audit_logger.setLevel(logging.INFO)
            audit_log = os.path.join(log_dir, 'mtp_audit.log')
            audit_handler = logging.handlers.RotatingFileHandler(
                audit_log, maxBytes=10_000_000, backupCount=10,
                encoding='utf-8'
            )
...
def get_logger(name: str = 'mtp') -> logging.Logger:
    """Get a named logger (child of 'mtp' root)."""
...
def audit_log(event: str, user: str = 'system', ip: str = '127.0.0.1',
    ...
    Args:
        event: Event type (QUERY, DATA_ACCESS, ERROR, LOGIN, etc.)
        user: User identifier
        ip: Client IP address
        details: Additional metadata (NEVER include PHI/PII)
    """
    audit = get_logger('mtp.audit')
...
def _sanitize_audit_details(details: Dict) -> Dict:
    """Remove any potential PHI from audit details. Keep only metadata."""
    safe = {}
...
# =============================================================================
# 2. PERSISTENT DATABASE CONNECTION POOL
# =============================================================================

class DatabasePool:
    """
    Thread-safe persistent SQLite connection pool.

    Loads CSV data ONCE at startup, then serves connections from the pool.
    Replaces the per-query CSV reload anti-pattern.

    Thread Safety:
        - Uses threading.local() for per-thread connections
        - SQLite WAL mode for concurrent reads
        - Read-only queries only (no writes after initial load)
    """
    ...
    def __init__(self):
        ...
        self._column_whitelist: Dict[str, set] = {}  # table -> {col1, col2, ...}
        self._load_lock = threading.Lock()
        self._logger = get_logger('mtp.db')
    ...
    def initialize(self, cfg: Dict[str, str]) -> Dict[str, Any]:
        """
        Load CSV data into a persistent SQLite database file.

        Called ONCE at application startup. Creates an on-disk SQLite DB
        in a temp location, loads all CSVs, then serves reads from it.

        Args:
            cfg: Configuration dict with BASE_DIR and/or RAW_DIR
        ...
        with self._load_lock:
            ...
            # Use a persistent DB file — try data/ first, fall back to /tmp
            ...
            try:
                ...
                import tempfile
                db_dir = os.path.join(tempfile.gettempdir(), 'mtp_healthcare')
                os.makedirs(db_dir, exist_ok=True)
                self._db_path = os.path.join(db_dir, 'healthcare_production.db')
                self._logger.info("DB directory fell back to: %s", db_dir)

            # Check if DB already exists, is recent (< 1 hour old), AND has tables
            if os.path.exists(self._db_path):
                db_age = time.time() - os.path.getmtime(self._db_path)
                if db_age < 3600:  # Less than 1 hour old
                    ...
                    if self._table_names:
                        ...
                        # Remove the empty/corrupt DB so we rebuild cleanly
                        try:
                            os.remove(self._db_path)
                        except OSError:
                            pass

            # Build fresh DB from CSVs
    ...
    def _load_csv_to_table(self, cursor: sqlite3.Cursor, csv_path: str,
                           table_name: str) -> int:
        """Load a single CSV file into a SQLite table. Returns row count."""
    ...
    def _create_indexes(self, cursor: sqlite3.Cursor) -> None:
        """Create indexes on common join/filter columns."""
        INDEX_COLUMNS = {
        ...
        for table, cols in INDEX_COLUMNS.items():
            ...
            for col in cols:
                try:
                    ...
                    pass  # Column might not exist in this dataset

    def _load_schema_from_db(self) -> None:
        """Load table/column schema from the DB for whitelist validation."""
    ...
    def get_connection(self) -> sqlite3.Connection:
        ...
        Returns a connection from the pool (one per thread).
    ...
    def execute_query(self, sql: str, timeout: int = 30) -> Tuple[bool, List[Dict], str]:
        """
        Execute a SQL query safely against the persistent database.
...
def validate_question(question: str) -> Tuple[bool, str, str]:
    ...
    # Reject potential SQL injection patterns in raw input
    INJECTION_PATTERNS = [
    ...
    for pattern in INJECTION_PATTERNS:
        if re.search(pattern, question, re.IGNORECASE):
            get_logger('mtp.security').warning(
...
class RateLimiter:
    ...
    Thread-safe. Tracks requests per client IP.
    """
    ...
    def __init__(self, requests_per_minute: int = 30, burst: int = 10):
        ...
        self._logger = get_logger('mtp.ratelimit')
...
def resolve_base_dir(cfg: Dict[str, str] = None) -> str:
    ...
    Priority:
      1. MTP_BASE_DIR environment variable
      2. cfg['BASE_DIR'] if it exists on disk
      3. Auto-detect from script location (../../ relative to this file)

    Returns:
        Absolute path to the mtp_demo/ base directory
    """
    # 1. Environment variable
    env_base = os.environ.get('MTP_BASE_DIR')
    if env_base and os.path.isdir(env_base):
        return os.path.abspath(env_base)

    # 2. Config file value (if it exists on disk)
    if cfg:
...
def resolve_config(cfg_input: Dict[str, str] = None) -> Dict[str, str]:
    ...
    Takes raw config (possibly with Mac-specific paths) and resolves
    all paths to work in the current environment.

    Args:
        cfg_input: Raw configuration dict

    Returns:
        Resolved configuration with valid paths
    """
    cfg = dict(cfg_input or {})
    base = resolve_base_dir(cfg)

    # Override all paths with resolved values
    cfg['BASE_DIR'] = base
...
# =============================================================================
# 7. STARTUP PREFLIGHT CHECKS
# =============================================================================

def preflight_check(cfg: Dict[str, str]) -> Dict[str, Any]:
    """
    Run startup preflight checks to ensure system is ready.

    Checks:
        1. Data directory exists and has CSV files
        2. Semantic catalog exists
        3. Required Python modules available
        4. Database can be loaded
        5. Log directory is writable

    Returns:
        Dict with check results and overall status
    """
    logger = get_logger('mtp.startup')
    ...
    # 4. Log directory (non-critical — falls back to /tmp if needed)
    ...
    if log_dir:
        try:
            ...
            # Log directory not writable — fall back to /tmp
            fallback_log = os.path.join('/tmp', 'mtp_logs')
            try:
    ...
    # Overall status — only data_dir is truly critical
