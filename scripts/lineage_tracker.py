...
logger = logging.getLogger('gpdm.lineage')

LINEAGE_SUBDIRS = ['staging', 'processed', 'dq', 'recon']
...
class LineageTracker:
    ...
    def __init__(self, cfg: Optional[Dict[str, Any]] = None):
        ...
        self.column_to_files: Dict[str, List[str]] = {}
    ...
    def _create_file_node_from_path(self, file_path: str, stage: str) -> None:
        ...
        if file_path.endswith(".csv"):
            try:
                ...
                logger.debug('Row count error: %s', e)
    ...
    def get_impact_analysis(self, column_hint: str) -> Dict[str, Any]:
        ...
        adj = defaultdict(set)
        for key, node in self.column_nodes.items():
            for dep in node.get('upstream', []) if isinstance(node, dict) else node.upstream:
                adj[dep].add(key)
            for down in node.get('downstream', []) if isinstance(node, dict) else node.downstream:
                adj[key].add(down)
