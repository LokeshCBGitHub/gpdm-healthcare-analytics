...
def _get_deep_nav():
    try:
        from dashboard_deep_insights import get_deep_insight_nav_items
        return get_deep_insight_nav_items()
    except Exception as e:
        logger.warning("Deep insights nav unavailable: %s", e)
        return ''

def _get_deep_pages():
    try:
        from dashboard_deep_insights import get_deep_insight_pages
        return get_deep_insight_pages()
    except Exception as e:
        logger.warning("Deep insights pages unavailable: %s", e)
        return ''

def _get_deep_js():
    try:
        from dashboard_deep_insights import get_deep_insight_js
        return get_deep_insight_js()
    except Exception as e:
        logger.warning("Deep insights JS unavailable: %s", e)
        return ''


def _get_advanced_nav():
    try:
        from dashboard_advanced_analytics import get_advanced_nav_items
        return get_advanced_nav_items()
    except Exception as e:
        logger.warning("Advanced analytics nav unavailable: %s", e)
        return ''

def _get_advanced_pages():
    try:
        from dashboard_advanced_analytics import get_advanced_pages
        return get_advanced_pages()
    except Exception as e:
        logger.warning("Advanced analytics pages unavailable: %s", e)
        return ''

def _get_advanced_js():
    try:
        from dashboard_advanced_analytics import get_advanced_js
        return get_advanced_js()
    except Exception as e:
        logger.warning("Advanced analytics JS unavailable: %s", e)
        return ''
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .db-toggle-btn { padding:4px 10px;background:var(--kp-gray-100);color:var(--kp-gray-600);border:1px solid var(--kp-gray-200);border-radius:4px;font-size:11px;font-weight:600;cursor:pointer;transition:var(--transition); }
        .db-toggle-btn:hover { background:var(--kp-blue-pale);color:var(--kp-blue);border-color:var(--kp-blue-light); }
        .db-toggle-active { background:var(--kp-blue);color:white;border-color:var(--kp-blue); }
        .db-toggle-active:hover { background:var(--kp-blue-light);color:white; }
...
''' + _get_deep_nav() + '''
''' + _get_advanced_nav() + '''
            <div class="nav-section" style="margin-top:8px;cursor:pointer;" onclick="toggleDomainMenu()">
                <span>Domain Dashboards</span>
                <span id="domainMenuArrow" style="font-size:9px;margin-left:4px;">&#9660;</span>
            </div>
            <div id="domainMenuItems" style="display:none;">
                <a class="nav-item" onclick="loadDomainDash('executive')" data-perm="dashboards" data-domain="executive"><span>Executive</span></a>
                <a class="nav-item" onclick="loadDomainDash('financial')" data-perm="dashboards" data-domain="financial"><span>Financial</span></a>
                <a class="nav-item" onclick="loadDomainDash('claims_severity')" data-perm="dashboards" data-domain="claims_severity"><span>Claims Severity</span></a>
                <a class="nav-item" onclick="loadDomainDash('clinical_outcomes')" data-perm="dashboards" data-domain="clinical_outcomes"><span>Clinical Outcomes</span></a>
                <a class="nav-item" onclick="loadDomainDash('demographics')" data-perm="dashboards" data-domain="demographics"><span>Demographics</span></a>
                <a class="nav-item" onclick="loadDomainDash('pharmacy')" data-perm="dashboards" data-domain="pharmacy"><span>Pharmacy</span></a>
                <a class="nav-item" onclick="loadDomainDash('forecasting')" data-perm="dashboards" data-domain="forecasting"><span>Forecasting</span></a>
                <a class="nav-item" onclick="loadDomainDash('population_health')" data-perm="dashboards" data-domain="population_health"><span>Population Health</span></a>
                <a class="nav-item" onclick="loadDomainDash('revenue_cycle')" data-perm="dashboards" data-domain="revenue_cycle"><span>Revenue Cycle</span></a>
                <a class="nav-item" onclick="loadDomainDash('provider_network')" data-perm="dashboards" data-domain="provider_network"><span>Provider Network</span></a>
                <a class="nav-item" onclick="loadDomainDash('referral_network')" data-perm="dashboards" data-domain="referral_network"><span>Referral Network</span></a>
                <a class="nav-item" onclick="loadDomainDash('appointment_access')" data-perm="dashboards" data-domain="appointment_access"><span>Appointments</span></a>
                <a class="nav-item" onclick="loadDomainDash('membership_intelligence')" data-perm="dashboards" data-domain="membership_intelligence"><span>Membership</span></a>
                <a class="nav-item" onclick="loadDomainDash('utilization')" data-perm="dashboards" data-domain="utilization"><span>Utilization</span></a>
                <a class="nav-item" onclick="loadDomainDash('quality')" data-perm="dashboards" data-domain="quality"><span>Quality (HEDIS)</span></a>
                <a class="nav-item" onclick="loadDomainDash('provider')" data-perm="dashboards" data-domain="provider"><span>Provider Analytics</span></a>
            </div>
            <!-- ═══ SESSIONS ═══ -->
    ...
    <div class="main">
        ...
        <div class="content">
            ...
            <div class="page active" id="page-chat">
                <div class="chat-container">
                    ...
                    <div class="chat-input-area">
                        <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;font-size:12px;color:var(--kp-gray-600);">
                            <span style="font-weight:600;">Database:</span>
                            <button class="db-toggle-btn db-toggle-active" id="dbToggle71k" onclick="switchChatDb('71k')">71K</button>
                            <button class="db-toggle-btn" id="dbToggle200k" onclick="switchChatDb('200k')">200K</button>
                            <button class="db-toggle-btn" id="dbToggle4m" onclick="switchChatDb('4m')">4M</button>
                            <span id="dbStatusLabel" style="margin-left:6px;font-size:11px;color:var(--kp-gray-500);">Active: 71K</span>
                        </div>
            ...
            <div class="page" id="page-dashboard">
                ...
                <div id="dashContent">
                    <div id="dbInfoBanner" style="background:linear-gradient(135deg,#e3f2fd,#f3e5f5);border:1px solid rgba(0,107,166,0.2);border-radius:var(--radius);padding:10px 14px;margin-bottom:14px;font-size:11px;color:var(--kp-gray-700);display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        <span><strong>Analytics Dashboard</strong> &mdash; Powered by 7-model forecasting engine with automated model selection</span>
                        <span id="dbInfoDetails" style="margin-left:auto;font-size:10px;color:var(--kp-gray-500);"></span>
                    </div>
            ...
            <!-- ═══ DOMAIN DASHBOARD VIEWER ═══ -->
            <div class="page" id="page-domaindb">
                <div style="margin-bottom:12px;display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                    <span id="domainDashTitle" style="font-size:16px;font-weight:700;color:#002855;">Domain Dashboard</span>
                    <div style="margin-left:auto;display:flex;align-items:center;gap:8px;">
                        <span style="font-size:12px;font-weight:600;color:#374151;">Database:</span>
                        <div id="dbToggle" style="display:flex;border:1px solid #d1d5db;border-radius:6px;overflow:hidden;">
                            <button onclick="switchDomainDb('71k')" id="dbBtn71k"
                                style="padding:6px 16px;font-size:12px;font-weight:600;border:none;cursor:pointer;background:#002855;color:white;">
                                71K Records</button>
                            <button onclick="switchDomainDb('4m')" id="dbBtn4m"
                                style="padding:6px 16px;font-size:12px;font-weight:600;border:none;cursor:pointer;background:white;color:#374151;">
                                4M Records</button>
                        </div>
                        <span id="domainDashLoading" style="display:none;font-size:12px;color:var(--kp-gray-500);">Loading...</span>
                    </div>
                </div>
                <div id="domainDashContent" style="background:white;border-radius:10px;border:1px solid #e5e7eb;overflow:hidden;min-height:600px;">
                    <iframe id="domainDashFrame" style="width:100%;border:none;min-height:800px;"
                        onload="resizeDomainFrame()"></iframe>
                </div>
            </div>
...
''' + _get_advanced_pages() + '''
    ...
    <div id="accessCodeOverlay" style="position:fixed;inset:0;z-index:10000;background:linear-gradient(135deg,#001a3a 0%,#002855 50%,#003d7a 100%);display:flex;align-items:center;justify-content:center;">
        <div style="background:white;border-radius:12px;padding:40px;width:380px;box-shadow:0 20px 60px rgba(0,0,0,0.4);">
            <div style="text-align:center;margin-bottom:24px;">
                <div style="font-size:28px;margin-bottom:8px;font-weight:700;color:#002855;">GPDM</div>
    ...
    function showPage(name) {
        ...
        const titles = { chat:'Ask Analytics', dashboard:'Dashboards', quality:'Quality Measures (HEDIS)', explore:'Data Explorer', system:'System Intelligence', forecast:'Forecasting', domaindb:'Domain Dashboard', executive:'Executive Intelligence', population:'Population Health', preventive:'Preventive Care ROI', dataquality:'Data Quality', survival:'Survival Analysis', healthequity:'Health Equity', causal:'Causal Analysis', fullscan:'Intelligence Scan' };
        ...
        if (name === 'executive' && typeof loadExecutiveIntel === 'function') loadExecutiveIntel();
        if (name === 'population' && typeof loadPopulationHealth === 'function') loadPopulationHealth();
        if (name === 'preventive' && typeof loadPreventiveCare === 'function') loadPreventiveCare();
        if (name === 'dataquality' && typeof loadDataQuality === 'function') loadDataQuality();
        if (name === 'survival' && typeof loadSurvival === 'function' && !_survLoaded) loadSurvival();
        if (name === 'healthequity' && typeof loadHealthEquity === 'function' && !_fairLoaded) loadHealthEquity();
        if (name === 'causal' && typeof loadCausal === 'function' && !_causalLoaded) loadCausal();
    }
    ...
    // ── Listen for messages from dashboard iframes (Suggested Next Steps) ──
    window.addEventListener('message', function(evt) {
        if (evt.data && evt.data.type === 'askAnalytics' && evt.data.question) {
            showPage('chat');
            document.getElementById('queryInput').value = evt.data.question;
            askQuestion();
        }
    });
    ...
    function buildResultCard(question, data, startCollapsed) {
        ...
        if (insights && Object.keys(insights).length > 0) {
            ...
            insightsSection += '<button onclick="document.getElementById(\\'' + insId + '\\').style.display=document.getElementById(\\'' + insId + '\\').style.display===\\'none\\'?\\'block\\':\\'none\\'" style="background:none;border:1px solid #D0E8D0;border-radius:6px;padding:4px 12px;font-size:11px;color:#2E7D32;cursor:pointer;font-weight:600">Statistical Insights</button>';
            ...
            insightsSection += '<div style="font-size:10px;font-weight:700;color:#1B5E20;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:8px">Statistical Analysis</div>';
    ...
    async function loadDashboard() {
        ...
        if (kpiResp && kpiResp.by_category) {
            ...
            // Populate DB info banner
            try {
                const sysResp = await authFetch(API + '/system/status').then(r => r.json()).catch(() => null);
                if (sysResp && sysResp.database) {
                    const db = sysResp.database;
                    const engines = sysResp.engines_active || {};
                    const activeEngines = Object.values(engines).filter(v => v).length;
                    const nlu = sysResp.builtin_nlu || {};
                    const dbEl = document.getElementById('dbInfoDetails');
                    if (dbEl) dbEl.innerHTML = 'DB: <strong>' + (db.path||'?') + '</strong> (' + (db.size_mb||0) + 'MB, ' + (db.total_rows||0).toLocaleString() + ' rows) &bull; ' + activeEngines + ' engines active'
                        + (nlu.active ? ' &bull; Query Engine: ' + (nlu.accuracy ? Math.round(nlu.accuracy*100) + '% acc' : 'active') : '');
                }
            } catch(e) {}
        }
    ...
    async function loadTablePreview() {
        ...
        // Use direct SQL for explorer (faster than full pipeline)
        try {
            const resp = await authFetch(API + '/run-sql', {
                method: 'POST',
                headers: authHeaders(),
    ...
    async function loadSystem() {
        ...
        try {
            ...
            html += '<div class="status-card" style="border-left:4px solid var(--kp-green);">'
                + '<h3 style="color:var(--kp-green);">&#9679; Connected Database (LIVE)</h3>'
                + '<div class="status-item"><span>Database</span><span class="status-value" style="color:var(--kp-blue);font-weight:700;">'+(db.path||'unknown')+'</span></div>'
                + '<div class="status-item"><span>Size</span><span class="status-value">'+(db.size_mb||0)+' MB</span></div>'
                + '<div class="status-item"><span>Tables</span><span class="status-value">'+(db.tables||0)+'</span></div>'
                + '<div class="status-item"><span>Total Rows</span><span class="status-value">'+(db.total_rows||0).toLocaleString()+'</span></div>';
            if (db.table_details) {
                db.table_details.forEach(t => {
                    html += '<div class="status-item" style="padding-left:12px;font-size:12px;"><span style="color:var(--kp-gray-500);">&#8226; '+t.name+'</span><span class="status-value">'+t.rows.toLocaleString()+' rows</span></div>';
                });
            }
            html += '</div>';
            ...
            if (ds.length > 0) {
                html += '<div class="status-card" style="border-left:4px solid var(--kp-teal);">'
                    + '<h3 style="color:var(--kp-teal);">Data Sources</h3>';
                ds.forEach(d => {
                    html += '<div class="status-item"><span>'+d.name+' <span style="font-size:10px;color:var(--kp-gray-400);">('+d.folder+')</span></span><span class="status-value">'+d.size_mb+' MB</span></div>';
                });
                html += '</div>';
            }

            // ── Active Engines ──
            const engines = status.engines_active || {};
            const activeCount = Object.values(engines).filter(v => v).length;
            const totalCount = Object.keys(engines).length;
            html += '<div class="status-card" style="border-left:4px solid var(--kp-blue);">'
                + '<h3 style="color:var(--kp-blue);">Active Engines ('+activeCount+'/'+totalCount+')</h3>';
            Object.entries(engines).forEach(([name, active]) => {
                const label = name.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
                const dot = active ? '<span style="color:var(--kp-green);">&#9679;</span>' : '<span style="color:var(--kp-red);">&#9675;</span>';
                html += '<div class="status-item"><span>'+dot+' '+label+'</span><span class="status-value" style="color:'+(active?'var(--kp-green)':'var(--kp-red)')+';">'+(active?'Active':'Inactive')+'</span></div>';
            });
            html += '</div>';

            // ── Query Engine ──
            const nlu = status.builtin_nlu || {};
            html += '<div class="status-card" style="border-left:4px solid '+(nlu.active?'var(--kp-green)':'var(--kp-red)')+';">'
                + '<h3 style="color:'+(nlu.active?'var(--kp-green)':'var(--kp-red)')+';">Query Processing Engine</h3>';
            if (nlu.active) {
                html += '<div class="status-item"><span>Status</span><span class="status-value" style="color:var(--kp-green);font-weight:700;">ACTIVE</span></div>'
                    + '<div class="status-item"><span>Accuracy</span><span class="status-value">'+(nlu.accuracy ? Math.round(nlu.accuracy*100)+'%' : 'N/A')+'</span></div>'
                    + '<div class="status-item"><span>Entity Values</span><span class="status-value">'+(nlu.entity_values||0)+'</span></div>'
                    + '<div class="status-item"><span>Index Terms</span><span class="status-value">'+(nlu.idf_terms||0)+'</span></div>'
                    + '<div class="status-item"><span>Schema Terms</span><span class="status-value">'+(nlu.schema_terms||0)+'</span></div>';
            } else {
                html += '<div class="status-item"><span>Status</span><span class="status-value" style="color:var(--kp-red);">Inactive</span></div>';
            }
            html += '</div>';

            // ── Pattern Recognition ──
            html += '<div class="status-card"><h3>Pattern Recognition</h3>'
                + '<div class="status-item"><span>Vocabulary Size</span><span class="status-value">'+(status.neural?.embedding_vocab||0)+'</span></div>'
                + '<div class="status-item"><span>Stored Patterns</span><span class="status-value">'+(status.neural?.hopfield_patterns||0)+'</span></div>'
                + '<div class="status-item"><span>Graph Nodes</span><span class="status-value">'+(status.neural?.gnn_nodes||0)+'</span></div></div>';

            // ── Learning Engine ──
            html += '<div class="status-card"><h3>Learning Engine</h3>'
            ...
            // ── Model Routing ──
            const routing = Object.entries(status.model_selector||{}).filter(([s,i])=>i.uses>0);
            if (routing.length > 0) {
                html += '<div class="status-card"><h3>Model Routing</h3>'
                    + routing.map(([s,i]) =>
            ...
            // ── CMS Knowledge Base ──
            html += '<div class="status-card"><h3>CMS Knowledge Base</h3>'
            ...
            // ── Schema ──
            html += '<div class="status-card"><h3>Schema</h3>'
            ...
            html += '<div class="status-card"><h3>Local Processing</h3>'
                + '<div class="status-item"><span>Status</span><span class="status-value" style="color:'+(llm.available?'var(--kp-green)':'var(--kp-gray-400)')+';">'+(llm.available?'Connected':'Not Available')+'</span></div>'
                + '<div class="status-item"><span>Type</span><span class="status-value">'+(llm.type||'none')+'</span></div></div>';

            container.innerHTML = html;
        } catch(e) {
            container.innerHTML = '<p style="color:var(--kp-red)">Could not load system status: ' + e.message + '</p>';
        }
    }
    ...
    function renderAllForecasts(data, container) {
        ...
        html += '<div style="grid-column:1/-1;background:linear-gradient(135deg,#e8f5e9,#f1f8e9);border:1px solid #c8e6c9;border-radius:8px;padding:12px 16px;display:flex;align-items:center;gap:10px;margin-bottom:4px;">'
            + '<div><strong style="font-size:13px;color:#2e7d32;">HIPAA Compliant On-Premise Forecasting</strong>'
            + '<div style="font-size:11px;color:#558b2f;margin-top:2px;">All forecasting is performed on-premise using 7 competing models: Holt-Winters, ARIMA, Exponential Smoothing, Linear Regression, Prophet-style, Bayesian Bootstrap, and Monte Carlo. '
            + 'Each metric is evaluated with train/test split and the best model is auto-selected by MAPE. No protected health information (PHI) is transmitted to any external service.</div></div></div>';
    ...
    function buildForecastCard(f, key, expanded) {
        ...
        const modelName = f.forecasts[f.recommended_model]?.explanation?.name || f.recommended_model || 'Auto';
        const modelsEval = f.summary.models_evaluated || Object.keys(f.forecasts).length;
        const modelsOk = f.summary.models_succeeded || modelsEval;
        const bestMape = f.summary.best_mape;
        const trend = f.summary.trend;
        const trendStr = f.summary.trend_strength;
        const seasonality = f.summary.seasonality;
        ...
        // Model competition bar (NEW)
        if (modelsEval > 1) {
            html += '<div style="background:linear-gradient(135deg,#f0f7ff,#e8f4fd);border:1px solid rgba(0,107,166,0.15);border-radius:8px;padding:10px 14px;margin-bottom:12px;">'
                + '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;">'
                + '<span style="font-size:11px;font-weight:600;color:var(--kp-blue);">Model Competition</span>'
                + '<span style="font-size:10px;color:var(--kp-gray-500);">' + modelsOk + '/' + modelsEval + ' models succeeded</span></div>'
                + '<div style="display:flex;gap:4px;flex-wrap:wrap;">';
            Object.entries(f.forecasts).forEach(([mkey, m]) => {
                const isWinner = mkey === f.recommended_model;
                const mape = m.params?.mape;
                const mapeStr = mape !== undefined ? mape.toFixed(1) + '%' : '?';
                const bgColor = isWinner ? 'var(--kp-blue)' : (mape && mape < 10 ? '#e8f5e9' : (mape && mape < 50 ? '#fff8e1' : '#fce4ec'));
                const fgColor = isWinner ? 'white' : (mape && mape < 10 ? '#2e7d32' : (mape && mape < 50 ? '#f57f17' : '#c62828'));
                html += '<span style="font-size:9px;background:' + bgColor + ';color:' + fgColor + ';padding:2px 8px;border-radius:10px;white-space:nowrap;'
                    + (isWinner ? 'font-weight:700;' : '') + '">'
                    + (m.explanation?.name || mkey) + ' ' + mapeStr
                    + (isWinner ? ' [Best]' : '') + '</span>';
            });
            html += '</div></div>';
        }

        // Data characteristics badges (NEW)
        if (trend || seasonality !== undefined) {
            html += '<div style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:10px;">';
            if (trend) {
                const tIcon = trend === 'increasing' ? '&#8599;' : trend === 'decreasing' ? '&#8600;' : '&#8594;';
                const tColor = trend === 'increasing' ? '#2e7d32' : trend === 'decreasing' ? '#c62828' : '#666';
                html += '<span style="font-size:10px;background:rgba(0,0,0,0.04);color:' + tColor + ';padding:2px 8px;border-radius:10px;">'
                    + tIcon + ' Trend: ' + trend + (trendStr ? ' (R\u00B2=' + trendStr.toFixed(3) + ')' : '') + '</span>';
            }
            if (seasonality !== undefined) {
                html += '<span style="font-size:10px;background:rgba(0,0,0,0.04);color:' + (seasonality ? '#1565c0' : '#888') + ';padding:2px 8px;border-radius:10px;">'
                    + (seasonality ? 'Seasonal patterns detected' : 'No seasonality') + '</span>';
            }
            if (bestMape !== undefined) {
                const mapeColor = bestMape < 5 ? '#2e7d32' : bestMape < 15 ? '#f57f17' : '#c62828';
                html += '<span style="font-size:10px;background:rgba(0,0,0,0.04);color:' + mapeColor + ';padding:2px 8px;border-radius:10px;">'
                    + 'Best MAPE: ' + bestMape.toFixed(1) + '%</span>';
            }
            html += '</div>';
        }
        ...
        html += '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">'
            + '<span style="font-size:11px;background:var(--kp-blue-pale,#E6F2FF);color:var(--kp-blue);padding:3px 10px;border-radius:12px;font-weight:500;">Winner: ' + modelName + (bestMape !== undefined ? ' (' + bestMape.toFixed(1) + '% MAPE)' : '') + '</span>'
            + '<span style="font-size:11px;background:#e8f5e9;color:#2e7d32;padding:3px 10px;border-radius:12px;">On-Premise</span>'
            + '<span style="font-size:11px;background:#fff3e0;color:#e65100;padding:3px 10px;border-radius:12px;">' + modelsEval + ' Models Evaluated</span>'
            + '</div>';
        html += '</div>';
        return html;
    }
    ...
    // ── Domain Dashboard Menu Toggle ──
    function toggleDomainMenu() {
        const items = document.getElementById('domainMenuItems');
        const arrow = document.getElementById('domainMenuArrow');
    ...
    // ── Domain Dashboards (inline, no new tabs) ──
    let _currentDomain = 'executive';
    let _currentDbSize = '71k';
    const _domainTitles = {
        executive: 'Executive Summary', financial: 'Financial Analytics',
        claims_severity: 'Claims Severity', clinical_outcomes: 'Clinical Outcomes',
        demographics: 'Demographics', pharmacy: 'Pharmacy Analytics',
        forecasting: 'Forecasting Intelligence', population_health: 'Population Health',
        revenue_cycle: 'Revenue Cycle', provider_network: 'Provider Network',
        referral_network: 'Referral Network', appointment_access: 'Appointment Access',
        membership_intelligence: 'Membership Intelligence', utilization: 'Utilization',
        quality: 'Quality (HEDIS)', provider: 'Provider Analytics'
    };

    function loadDomainDash(domain) {
        _currentDomain = domain;
        // Show the domaindb page via normal showPage
        showPage('domaindb');
        // Update title
        document.getElementById('domainDashTitle').textContent = _domainTitles[domain] || domain;
        // Highlight the correct nav item
        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active'));
        const navEl = document.querySelector('.nav-item[data-domain="' + domain + '"]');
        if (navEl) navEl.classList.add('active');
        // Load the iframe
        _loadDomainFrame();
    }

    let _activeChatDb = '71k';
    async function switchChatDb(dbKey) {
        const label = document.getElementById('dbStatusLabel');
        const btns = ['71k','200k','4m'];
        btns.forEach(k => {
            const b = document.getElementById('dbToggle' + k.replace('k','k').charAt(0).toUpperCase() + k.slice(1));
            if (!b) {
                const fallback = document.getElementById('dbToggle' + k);
                if (fallback) { fallback.classList.remove('db-toggle-active'); }
                return;
            }
            b.classList.remove('db-toggle-active');
        });
        const btn71 = document.getElementById('dbToggle71k');
        const btn200 = document.getElementById('dbToggle200k');
        const btn4 = document.getElementById('dbToggle4m');
        if (btn71) btn71.classList.remove('db-toggle-active');
        if (btn200) btn200.classList.remove('db-toggle-active');
        if (btn4) btn4.classList.remove('db-toggle-active');
        const activeBtn = document.getElementById('dbToggle' + dbKey);
        if (activeBtn) activeBtn.classList.add('db-toggle-active');
        if (label) label.textContent = 'Switching...';
        try {
            const resp = await fetch('/api/switch-db', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({db: dbKey, token: authToken})
            });
            const data = await resp.json();
            if (data.ok) {
                _activeChatDb = dbKey;
                if (label) label.textContent = 'Active: ' + dbKey.toUpperCase() + ' (' + (data.claims_count || 0).toLocaleString() + ' claims)';
            } else {
                if (label) label.textContent = 'Error: ' + (data.error || 'Unknown');
                setTimeout(() => { if (label) label.textContent = 'Active: ' + _activeChatDb.toUpperCase(); }, 3000);
            }
        } catch(e) {
            if (label) label.textContent = 'Error switching DB';
            setTimeout(() => { if (label) label.textContent = 'Active: ' + _activeChatDb.toUpperCase(); }, 3000);
        }
    }

    function switchDomainDb(dbSize) {
        _currentDbSize = dbSize;
        // Toggle button styles
        const btn71 = document.getElementById('dbBtn71k');
        const btn4m = document.getElementById('dbBtn4m');
        if (dbSize === '71k') {
            btn71.style.background = '#002855'; btn71.style.color = 'white';
            btn4m.style.background = 'white'; btn4m.style.color = '#374151';
        } else {
            btn4m.style.background = '#002855'; btn4m.style.color = 'white';
            btn71.style.background = 'white'; btn71.style.color = '#374151';
        }
        _loadDomainFrame();
    }

    function _loadDomainFrame() {
        const frame = document.getElementById('domainDashFrame');
        const loading = document.getElementById('domainDashLoading');
        loading.style.display = '';
        frame.src = '/dashboards/' + _currentDbSize + '/' + _currentDomain;
    }

    function resizeDomainFrame() {
        const frame = document.getElementById('domainDashFrame');
        const loading = document.getElementById('domainDashLoading');
        ...
        try {
            ...
            // Cross-origin — fallback to large height
